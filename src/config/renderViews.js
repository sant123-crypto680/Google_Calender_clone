...
export default function renderViews(context, datepickerContext, store) {
  ...
  async function handleBtnMainMenu() {
    ...
    if (currentSidebarState === 'hide') {
      if (toggleForm) toggleForm.onclick = handleForm;
      if (sbToggleForm) sbToggleForm.onclick = null;
      if (sbToggleSubBtn) sbToggleSubBtn.onclick = null;
      if (sbDatepicker) sbDatepicker.onclick = null;
      if (sbFooter) sbFooter.onmousedown = null;
      if (sbCategories) sbCategories.onmousedown = null;
      setTimeout(() => {
        if (sbDatepickerBody) sbDatepickerBody.innerText = '';
        if (sbCategoriesWrapper) sbCategoriesWrapper.innerText = '';
      }, 100);
      if (viewsContainer) viewsContainer.classList.remove('container__calendars-sb-active');
      if (sidebar) sidebar.classList.add('hide-sidebar');
      if (toggleForm) toggleForm.classList.remove('hide-toggle--form');

      if (prevnext) prevnext.classList.remove('prevnext-inactive');
      if (dateTimeBtn) dateTimeBtn.classList.remove('prevnext-inactive');
      ...
      if (prevnext) prevnext.removeAttribute('tabindex');
      if (dateTimeBtn) dateTimeBtn.removeAttribute('tabindex');
      if (listviewBody) listviewBody.removeAttribute('style');
      // clear categories/datepicker content when inactive
    } else {
      if (toggleForm) toggleForm.onclick = null;
      if (sbToggleForm) sbToggleForm.onclick = handleForm;
      if (sbToggleSubBtn) sbToggleSubBtn.onclick = handleToggleSubmenu;

      if (context.getComponent() === 'list' && listviewBody) {
        listviewBody.style.width = '100%';
        listviewBody.style.marginLeft = '0';
      }

      if (viewsContainer) viewsContainer.classList.add('container__calendars-sb-active');
      if (sidebar) sidebar.classList.remove('hide-sidebar');
      if (toggleForm) toggleForm.classList.add('hide-toggle--form');

      // Keep navigation buttons active even when sidebar is open
