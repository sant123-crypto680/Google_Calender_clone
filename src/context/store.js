...
class Store {
  constructor() {
    ...
    // Initialize holidays category
    if (!this.ctg.holidays) {
      this.ctg.holidays = { color: '#9E69AF', active: true };
  ...
  getEntry(id) {
    // First check regular store
    const entry = this.store.find((entry) => entry.id === id);
    if (entry) return entry;

    // If not found, check holidays
  ...
  getDayEntries(day) {
    ...
    // Merge with holidays for the day
    const dayStart = new Date(day.getFullYear(), day.getMonth(), day.getDate(), 0, 0, 0);
    const dayEnd = new Date(day.getFullYear(), day.getMonth(), day.getDate(), 23, 59, 59);
    dayEntries = this.mergeHolidaysWithEntries(dayEntries, dayStart, dayEnd);
  ...
  getMonthEntries(montharr) {
    ...
    // Merge with holidays if available
    return this.mergeHolidaysWithEntries(filteredEntries, montharr[0], montharr.at(-1));
  }
  ...
  async loadHolidaysForYear(year) {
    ...
    if (this.holidaysLoaded && this.holidays.length > 0) {
      const firstHoliday = new Date(this.holidays[0].start);
      if (firstHoliday.getFullYear() === year) {
    ...
    try {
      ...
      // Convert holiday format to match entry format
      this.holidays = holidays.map(holiday => ({
        ...holiday,
        completed: false,
        isHoliday: true,
        isReadOnly: true,
        start: new Date(holiday.start).toISOString(),
        end: new Date(holiday.end).toISOString(),
      }));

      this.holidaysLoaded = true;
      this.currentHolidayYear = year;
    } catch (error) {
  ...
  /**
   * Merge holidays with entries
   * @param {Array} entries - Regular entries
   * @param {Date} startDate - Start date for holiday filtering
   * @param {Date} endDate - End date for holiday filtering
   * @returns {Array} Merged entries and holidays
   */
  mergeHolidaysWithEntries(entries, startDate, endDate) {
  ...
  getWeekEntries(week) {
    ...
    // Merge with holidays for the week
    entries = this.mergeHolidaysWithEntries(entries, start, end);
